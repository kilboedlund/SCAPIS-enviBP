...
table1 <- 
  ...
  mutate(across(-c('var', 'n_miss'), \(x) round(x, 2)))
...
df_1 %>% summarise(quantile(pulse, c(.33, .5, .67), na.rm = T))
...
table2 <- 
  left_join(
    summarise(df_1 %>% select(o3_24h, temp_24h, pm10_24h, no2_24h) %>% ungroup, 
              across(everything(),
                     \(x) paste0(sprintf('%.1f', median(x, na.rm = T)), ' (',
                                 sprintf('%.1f', quantile(x, .33, na.rm = T)), ', ',
                                 sprintf('%.1f', quantile(x, .67, na.rm = T)), ')')
              ) %>% pivot_longer(cols = everything(), names_to = 'exposure', values_to = 'all')
    ),
    summarise(df_1 %>% select(site, o3_24h, temp_24h, pm10_24h, no2_24h) %>% group_by(site), 
              across(everything(),
                     \(x) paste0(sprintf('%.1f', median(x, na.rm = T)), ' (',
                                 sprintf('%.1f', quantile(x, .33, na.rm = T)), ', ',
                                 sprintf('%.1f', quantile(x, .67, na.rm = T)), ')')
