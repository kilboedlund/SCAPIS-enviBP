...
df_3 <- df_2 %>% filter(hpt == 'no', hpt_med == 'no')
...
fit_hr_temp_o3_pm10_spline <-
  lme4::lmer(
    formula = pulse ~ ns(temp_24h, 3) + ns(o3_24h, 3) + ns(pm10_24h) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
p2sbp <- 
  ...
  geom_line(aes(y = fit), colour = 'darkorange', linetype = 'dashed') +
  geom_line(aes(y = fit_temp), colour = 'darkgreen', linetype = 'dotted') +
  geom_line(aes(y = fit_temp_pm10), colour = 'darkblue') +
...
fit_sbp_o3_temp_inter <-
  lme4::lmer(
    formula = sbp ~ ns(o3_24h, 3)*temp_24h + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_dbp_o3_temp_inter <-
  lme4::lmer(
    formula = dbp ~ ns(o3_24h, 3)*temp_24h + ns(pm10_24h) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_pp_o3_temp_inter <-
  lme4::lmer(
    formula = pp ~ ns(o3_24h, 3)*temp_24h + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_hr_o3_temp_inter <-
  lme4::lmer(
    formula = pulse ~ ns(o3_24h, 3)*temp_24h + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
ggsave(here::here('figures', paste0(Sys.Date(), '_fig1_nohpt.svg')), width = 14, height = 9)
ggsave(here::here('figures', paste0(Sys.Date(), '_fig1_nohpt.png')), width = 14, height = 9)
...
tableS2 <-
  bind_rows(
    fit_sbp_temp_o3_pm10_linear %>% 
      ...
      mutate(outcome = 'sbp',
             across(c(estimate, conf.low, conf.high), \(x) x / sd(df_1$temp_24h, na.rm = T))),
    fit_dbp_temp_o3_pm10_linear %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'temp_24h')) %>% 
      mutate(outcome = 'dbp',
