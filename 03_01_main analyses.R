...
newdat <- expand.grid(o3_24h = seq(quantile(df_3$o3_24h, .05, na.rm = T),
                                   ...
                                   length.out = 100),
                      temp_24h_t = factor(c(1,2,3))) %>% 
  mutate(pm10_24h = 0,
         no2_24h = 0,
         temp_24h = 0,
         o3_24h_t = factor('1', levels = c(1,2,3)),
         sex = 'FEMALE',
         age = 55,
         diab = 'no',
         whr = .9,
...
fit_sbp_temp_temp_o3_linear_inter <-
  lme4::lmer(
...
fit_sbp_o3_o3_temp_linear_inter <-
  lme4::lmer(
...
p3sbp <- 
  ggplot(newdat_sbp %>% 
         mutate(temp_24h_t =factor(temp_24h_t,
                         labels = c('Coldest tertile',
                                    'Middle tertile',
                                    'Warmest tertile'))),
       aes(x = o3_24h*sd(df_1$o3_24h, na.rm = T) + mean(df_1$o3_24h, na.rm = T),
           group = temp_24h_t)) +
  ...
  geom_ribbon(aes(ymin = lcl, ymax = ucl,
                  fill = temp_24h_t, 
                  colour = temp_24h_t), linewidth = .1, alpha = .1) + 
  geom_line(aes(y = fit, 
                colour = temp_24h_t,
                linetype = temp_24h_t)) +
  scale_x_continuous(expand = c(0,0), breaks = 2:7*10) +
...
fit_dbp_temp_temp_o3_linear_inter <-
  lme4::lmer(
...
fit_dbp_o3_o3_temp_linear_inter <-
  lme4::lmer(
...
fit_pp_temp_temp_o3_linear_inter <-
  lme4::lmer(
...
fit_pp_o3_o3_temp_linear_inter <-
  lme4::lmer(
...
fit_hr_temp_temp_o3_linear_inter <-
  lme4::lmer(
...
fit_hr_o3_o3_temp_linear_inter <-
  lme4::lmer(
...
ggsave(here::here('figures', paste0(Sys.Date(), '_fig1.svg')), width = 14, height = 9)
ggsave(here::here('figures', paste0(Sys.Date(), '_fig1.png')), width = 14, height = 9)
...
table3 <- bind_rows(
  ...
  fit_sbp_temp_o3_linear_inter %>% 
    ...
    mutate(outcome = 'sbp',
           restriction = 'all',
           p_int = anova(fit_sbp_temp_o3_linear_inter, fit_sbp_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  fit_sbp_temp_temp_o3_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'temp_24h')) %>% 
    mutate(outcome = 'sbp',
           restriction = 'all',
           p_int = anova(fit_sbp_temp_temp_o3_linear_inter, fit_sbp_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  ...
  fit_dbp_temp_o3_linear_inter %>% 
    ...
    mutate(outcome = 'dbp',
           restriction = 'all',
           p_int = anova(fit_dbp_temp_o3_linear_inter, fit_dbp_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  fit_dbp_temp_temp_o3_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'temp_24h')) %>% 
    mutate(outcome = 'dbp',
           restriction = 'all',
           p_int = anova(fit_dbp_temp_temp_o3_linear_inter, fit_dbp_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  ...
  fit_pp_temp_o3_linear_inter %>% 
    ...
    mutate(outcome = 'pp',
           restriction = 'all',
           p_int = anova(fit_pp_temp_o3_linear_inter, fit_pp_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  fit_pp_temp_temp_o3_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'temp_24h')) %>% 
    mutate(outcome = 'pp',
           restriction = 'all',
           p_int = anova(fit_pp_temp_temp_o3_linear_inter, fit_pp_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  ...
  fit_hr_temp_o3_linear_inter %>% 
    ...
    mutate(outcome = 'hr',
           restriction = 'all',
           p_int = anova(fit_hr_temp_o3_linear_inter, fit_hr_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  fit_hr_temp_temp_o3_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'temp_24h')) %>% 
    mutate(outcome = 'hr',
           restriction = 'all',
           p_int = anova(fit_hr_temp_temp_o3_linear_inter, fit_hr_temp_o3_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
  ...
  fit_sbp_o3_temp_linear_inter %>% 
    ...
    mutate(outcome = 'sbp',
           restriction = 'all',
           p_int = anova(fit_sbp_o3_temp_linear_inter, fit_sbp_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
  fit_sbp_o3_o3_temp_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'o3_24h')) %>% 
    mutate(outcome = 'sbp',
           restriction = 'all',
           p_int = anova(fit_sbp_o3_o3_temp_linear_inter, fit_sbp_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
  ...
  fit_dbp_o3_temp_linear_inter %>% 
    ...
    mutate(outcome = 'dbp',
           restriction = 'all',
           p_int = anova(fit_dbp_o3_temp_linear_inter, fit_dbp_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
  fit_dbp_o3_o3_temp_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'o3_24h')) %>% 
    mutate(outcome = 'dbp',
           restriction = 'all',
           p_int = anova(fit_dbp_o3_o3_temp_linear_inter, fit_dbp_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
  ...
  fit_pp_o3_temp_linear_inter %>% 
    ...
    mutate(outcome = 'pp',
           restriction = 'all',
           p_int = anova(fit_pp_o3_temp_linear_inter, fit_pp_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
  fit_pp_o3_o3_temp_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'o3_24h')) %>% 
    mutate(outcome = 'pp',
           restriction = 'all',
           p_int = anova(fit_pp_o3_o3_temp_linear_inter, fit_pp_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
  ...
  fit_hr_o3_temp_linear_inter %>% 
    ...
    mutate(outcome = 'hr',
           restriction = 'all',
           p_int = anova(fit_hr_o3_temp_linear_inter, fit_hr_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
  fit_hr_o3_o3_temp_linear_inter %>% 
    tidy(conf.int = T) %>% 
    filter(str_detect(term, 'o3_24h')) %>% 
    mutate(outcome = 'hr',
           restriction = 'all',
           p_int = anova(fit_hr_o3_o3_temp_linear_inter, fit_hr_o3_temp_linear)$`Pr(>Chisq)`[2],
           across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T)))
  ) %>% 
