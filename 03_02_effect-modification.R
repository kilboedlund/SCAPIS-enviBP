...
# * Effect modification by SCORE2 ----------------------------------------------
# 1 Single exposure temperature ------------------------------------------------
df_3 <- df_2 %>% filter(diab == 'no') %>% mutate(score2 = ntile(score2_cal, 3) %>% factor)

## SBP -------------------------------------------------------------------------
fit_sbp_temp_o3_linear_score2 <-
  lme4::lmer(
    formula = sbp ~ temp_24h:score2 + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

fit_sbp_temp_o3_linear <-
  lme4::lmer(
    formula = sbp ~ temp_24h + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

## DBP -------------------------------------------------------------------------
fit_dbp_temp_o3_linear_score2 <-
  lme4::lmer(
    formula = dbp ~ temp_24h:score2 + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

fit_dbp_temp_o3_linear <-
  lme4::lmer(
    formula = dbp ~ temp_24h + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

## PP --------------------------------------------------------------------------
fit_pp_temp_o3_linear_score2 <-
  lme4::lmer(
    formula = pp ~ temp_24h : score2 + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

fit_pp_temp_o3_linear <-
  lme4::lmer(
    formula = pp ~ temp_24h + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

## HR --------------------------------------------------------------------------
fit_hr_temp_o3_linear_score2 <-
  lme4::lmer(
    formula = pulse ~ temp_24h : score2 + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

fit_hr_temp_o3_linear <-
  lme4::lmer(
    formula = pulse ~ temp_24h + score2 + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

# 2 Single exposure O3 ---------------------------------------------------------
## SBP -------------------------------------------------------------------------
fit_sbp_o3_temp_linear_score2 <-
  lme4::lmer(
    formula = sbp ~ o3_24h : score2 + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

fit_sbp_o3_temp_linear <-
  lme4::lmer(
    formula = sbp ~ o3_24h + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

## DBP -------------------------------------------------------------------------
fit_dbp_o3_temp_linear_score2 <-
  lme4::lmer(
    formula = dbp ~ o3_24h : score2 + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

fit_dbp_o3_temp_linear <-
  lme4::lmer(
    formula = dbp ~ o3_24h + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

## PP -------------------------------------------------------------------------
fit_pp_o3_temp_linear_score2 <-
  lme4::lmer(
    formula = pp ~ o3_24h : score2 + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

fit_pp_o3_temp_linear <-
  lme4::lmer(
    formula = pp ~ o3_24h + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )

## HR --------------------------------------------------------------------------
fit_hr_o3_temp_linear_score2 <-
  lme4::lmer(
    formula = pulse ~ o3_24h : score2 + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_hr_o3_temp_linear <-
  lme4::lmer(
    formula = pulse ~ o3_24h + score2 + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
table4 <-
  bind_rows(
    fit_sbp_temp_o3_linear_score2 %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'temp_24h:score2')) %>% 
      mutate(outcome = 'sbp',
             p_int = anova(fit_sbp_temp_o3_linear, fit_sbp_temp_o3_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
    fit_dbp_temp_o3_linear_score2 %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'temp_24h:score2')) %>% 
      mutate(outcome = 'dbp',
             p_int = anova(fit_dbp_temp_o3_linear, fit_dbp_temp_o3_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
    fit_pp_temp_o3_linear_score2 %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'temp_24h:score2')) %>% 
      mutate(outcome = 'pp',
             p_int = anova(fit_pp_temp_o3_linear, fit_pp_temp_o3_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
    fit_hr_temp_o3_linear_score2 %>% 
      tidy(conf.int = T) %>%
      filter(str_detect(term, 'temp_24h:score2')) %>% 
      mutate(outcome = 'hr',
             p_int = anova(fit_hr_temp_o3_linear, fit_hr_temp_o3_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$temp_24h, na.rm = T))),
    fit_sbp_o3_temp_linear_score2 %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'o3_24h:score2')) %>% 
      mutate(outcome = 'sbp',
             p_int = anova(fit_sbp_o3_temp_linear, fit_sbp_o3_temp_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
    fit_dbp_o3_temp_linear_score2 %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'o3_24h:score2')) %>% 
      mutate(outcome = 'dbp',
             p_int = anova(fit_dbp_o3_temp_linear, fit_dbp_o3_temp_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
    fit_pp_o3_temp_linear_score2 %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'o3_24h:score2')) %>% 
      mutate(outcome = 'pp',
             p_int = anova(fit_pp_o3_temp_linear, fit_pp_o3_temp_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T))),
    fit_hr_o3_temp_linear_score2 %>% 
      tidy(conf.int = T) %>% 
      filter(str_detect(term, 'o3_24h:score2')) %>% 
      mutate(outcome = 'hr',
             p_int = anova(fit_hr_o3_temp_linear, fit_hr_o3_temp_linear_score2)$`Pr(>Chisq)`[2],
             across(c(estimate, conf.low, conf.high), \(x) 10 * x / sd(df_1$o3_24h, na.rm = T)))
  ) %>% 
  separate(term, into = c('main', 'inter'), sep = ':') %>% 
  mutate(estimate = paste0(sprintf('%.2f', estimate), ' (',
...
fit_pp_temp_o3_linear_hpt <-
  lme4::lmer(
    formula = pp ~ temp_24h : hpt + hpt + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_hr_temp_o3_linear_hpt <-
  lme4::lmer(
    formula = pulse ~ temp_24h : hpt + hpt + ns(o3_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_sbp_o3_temp_linear_hpt <-
  lme4::lmer(
    formula = sbp ~ o3_24h : hpt + hpt + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_dbp_o3_temp_linear_hpt <-
  lme4::lmer(
    formula = dbp ~ o3_24h : hpt + hpt + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_pp_o3_temp_linear_hpt <-
  lme4::lmer(
    formula = pp ~ o3_24h : hpt + hpt + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
...
fit_hr_o3_temp_linear_hpt <-
  lme4::lmer(
    formula = pulse ~ o3_24h : hpt + hpt + ns(temp_24h, 3) + ns(pm10_24h, 3) + ns(no2_24h, 3) + sex + age + diab + whr + smo + alc + phy + ses + year + month + wday + (1|site),
    data = df_3
  )
